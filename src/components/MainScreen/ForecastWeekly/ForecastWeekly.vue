<template>
  <ContainerBig>
    <template #header
      ...
      {{ $store.getters.t.forecast }} ({{ forecast.length }}
      {{ $store.getters.t.days }})
    </template>
...
export default {
  ...
  methods: {
    passDayOfWeek(value) {
      ...
      if (value === 0)
        return {
          full: this.$store.getters.t.today,
          short: this.$store.getters.t.today,
        };
      else
        return {
          full: this.$store.getters.t.daysOfWeek[
            this.daysOfWeek[(value + day) % 7]
          ],
          short:
            this.$store.getters.t.daysOfWeek[
              this.daysOfWeek[(value + day) % 7].slice(0, 3)
            ],
        };
    },
    passIconSrc(day) {
